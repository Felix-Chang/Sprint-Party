        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-[#9CA3AF]">
              ...
              {TIME_LIMIT_LABEL[t.difficulty]} limit
            </p>
          </div>
          {selected === t.id && (
            <span className="text-[#1A1A2E] text-base">✓</span>
  ...
  async function handleConfirm() {
    ...
    try {
      if (puId === "freeze") {
        ...
        if ((target.power_ups ?? []).includes("shield")) {
          const shieldStripped = (target.power_ups ?? []).filter(
            (p) => p !== "shield",
          );
          await supabase
            .from("players")
            .update({ power_ups: shieldStripped })
            .eq("user_id", selectedTarget)
            .eq("room_id", roomId);
          await removeFromInventory();
          showToast("🛡️ Blocked! Their shield deflected your attack.", "error");
          onClose();
          return;
        }
        ...
        const durationMs =
          POWER_UPS.double_or_nothing.timeLimits[task.difficulty];
        ...
        const updatedTasks = (player.tasks ?? []).map((t) =>
          t.id === selectedTask
            ? {
                ...
                doubleOrNothingExpiresAt: expiresAt,
              }
            : t,
        );
        await supabase
        ...
        const until = new Date(
          Date.now() + POWER_UPS.ghost_mode.duration,
        ).toISOString();
        await supabase
          .from("players")
        ...
        await removeFromInventory();
        showToast(
          "Sprint Boost active! Next 3 completions +50 pts 🚀",
          "success",
        );
      } else if (puId === "reroll") {
        await removeFromInventory();
        ...
        // shield is passive — stays in inventory until an attack hits
        showToast(
          "Shield is active. It will block the next attack targeting you. 🛡️",
          "info",
        );
      }
    } finally {
